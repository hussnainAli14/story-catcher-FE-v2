...
import { storyAPI, API_BASE_URL } from '@/lib/api';
...
const Storyboard: React.FC<StoryboardProps> = ({
  ...
  const downloadVideo = async (url: string) => {
    ...
    if (downloadUrl) {
      // On mobile/iOS, direct navigation is more reliable for downloads when Content-Disposition is attachment
    ...
    try {
      ...
      try {
        // Calculate API base URL dynamically to ensure it works on mobile/LAN
        let apiBase = '';

        // 1. Try to use the centralized configuration first
        if (API_BASE_URL && !API_BASE_URL.includes('localhost')) {
          apiBase = API_BASE_URL;
        }
        // 2. If we are in browser, construct from current hostname (handles LAN/Mobile)
        ...
        // Use direct navigation for proxy download as well
        // This is the most robust method for iOS/Mobile
